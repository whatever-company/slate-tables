import { getCellColspan, withSpan } from '../utils.js'
...
export default function deleteColumnAtKey({ blocks, saveColumns }, editor, key) {
	...
	editor.withoutNormalizing(() => {
		if (saveColumns) {
			// Copied: the array is shared with the values kept in the history
		...
		for (let y = 0; y < table.rows.size; y++) {
			...
			if (getCellColspan(lastCell) > 1) {
				// The column runs through a merged cell: it only gets narrower
				editor.setNodeByKey(lastCell.key, {
					data: withSpan(lastCell.data, 'colspan', getCellColspan(lastCell) - 1)
				})
			} else {
