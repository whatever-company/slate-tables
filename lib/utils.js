...
function createMatrix(table) {
	const matrix = []
	table.nodes.forEach((row, rowIdx) => {
		matrix[rowIdx] ??= []
		let colIdx = 0
		for (const cell of row.nodes) {
			...
			for (let spanIdx = rowIdx; spanIdx < rowIdx + getCellRowspan(cell); spanIdx++) {
				const matrixRow = (matrix[spanIdx] ??= [])
				for (let colSpanIdx = 0; colSpanIdx < colspan; colSpanIdx++) {
					// Insert cell at first empty position
					let i = 0
					while (matrixRow[colIdx + colSpanIdx + i]) {
						i++
					}
					const virtual = colSpanIdx > 0 || spanIdx !== rowIdx
					matrixRow[colIdx + colSpanIdx + i] = { ref: cell, virtual }
				}
			}
			colIdx += colspan
		}
