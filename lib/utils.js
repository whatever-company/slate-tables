...
const Cell = new Immutable.Record({ virtual: false, ref: null })

function createMatrix(table) {
	let matrix = new Immutable.List()
	const rows = table.nodes
	let rowIdx = 0
	for (const row of rows) {
		let colIdx = 0
		if (!matrix.get(rowIdx)) {
			matrix = matrix.set(rowIdx, new Immutable.List())
		}
		for (const cell of row.nodes) {
			let colSpanIdx
			for (let spanIdx = rowIdx; spanIdx < rowIdx + getCellRowspan(cell); spanIdx++) {
				if (!matrix.get(spanIdx)) {
					matrix = matrix.set(spanIdx, new Immutable.List())
				}
...
const Coordinates = new Immutable.Record({
	x: null,
	y: null
})

export class Table extends Immutable.Record({
	tableBlock: null,
	rowBlock: null,
	cellBlock: null,
	contentBlock: null,
	...
	getCellCoordinates(cell) {
		...
		return new Immutable.List(coords)
	}
