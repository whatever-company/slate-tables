...
export default function (options) {
	...
	return function normalizeNode(node, editor, next) {
		...
		const rowsMissingColumns = matrix.filter(row => row.length !== maxColumns)
		if (rowsMissingColumns.length !== 0) {
			// Check if only one empty cell is causing a problem of columns
			const lastRowEntry = matrix.filter(row => row.length === maxColumns).map(row => row.at(-1))
			const lastCell = lastRowEntry.at(-1)
			if (lastRowEntry.length === 1 && lastCell && getCellRowspan(lastCell.ref) === 1 && !lastCell.ref.text) {
				return () => {
				...
				editor.withoutNormalizing(() => {
					editor.withoutSaving(() => {
						...
						matrix.forEach((matrixRow, rowIndex) => {
							if (matrixRow.length === maxColumns) return
							...
							const missingCells = Array.from({ length: maxColumns - matrixRow.length }, () =>
